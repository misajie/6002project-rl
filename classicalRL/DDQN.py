...
class DDQN(Net):
    ...
    def learn(self):
        ...
        b_a = Variable(torch.LongTensor(b_memory[:, (N_STATES):(N_STATES+1)].astype(int))) # 踩大类，这里的tensor维度传输绝对要注意
        b_r = Variable(torch.FloatTensor(b_memory[:, (N_STATES + 1):(N_STATES+2)]))  # 这里不需要管外部环境的具体分数
        b_s_ = Variable(torch.FloatTensor(b_memory[:, -N_STATES:]))

        # update eval_net using batch data
        # DDQN change the estimate of next action with maximum est from evaluate net
        dd_action = torch.unsqueeze(torch.max(self.eval_net(b_s_), 1)[1],1)
        q_next = self.target_net(b_s).detach().gather(1, dd_action)  # 被tensor维度和gather搞到自闭
        ...
        loss = self.loss_func(q_eval, q_target)  # 优化td-target
